...
//            throw new Exception('Erro ao executar a consulta: ' . pg_last_error($this->con));
    ...
    private function doConnect() {
        ...
        if(IS_DEV) {
            $connectionString = "host={$config['host_dev']} dbname={$config['dbname_dev']} user={$config['user_dev']} password={$config['password_dev']}";
        }else{
            $connectionString = "host={$config['host']} dbname={$config['dbname']} user={$config['user']} password={$config['password']}";
        }
